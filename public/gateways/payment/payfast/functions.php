...
function carepro_pay($output){
   ...
   $data = $output;
   $data['tid'] = uniqid().'-'.$output['invoice']->id;
 ...
 //   $return_url = $this->url()->fromRoute('application/default',['controller'=>'callback','action'=>'payfast','id'=>$data['tid']],['force_canonical' => true]);//$this->url->link('checkout/success','',$config->get('config_use_ssl')?'SSL':'NONSSL');
    $return_url = route('user.billing.invoices');

    $cancel_url = route('user.invoice.cart');;// $this->url->link('checkout/checkout','',$config->get('config_use_ssl')?'SSL':'NONSSL');
...
function carepro_ipn(){
    try{
        ...
        $id = request()->get('tid');
        $invoiceId = substr($id,stripos($id,'-')+1);
        $invoice = \App\Invoice::find($invoiceId);
